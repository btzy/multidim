...
TEST_CASE("array assignment", "[2d][dynarray][assignment]") {
	multidim::array<multidim::inner_array<int, 3>, 3> arr2;
	// TODO: change to better fill()
	for (int i = 0; i < 3; ++i) arr2[i].fill(0); // prevents undefined behaviour of the assignment below
	arr2[1][1] = 45;
	multidim::array<multidim::inner_array<int, 3>, 3> tmp;
	tmp = arr2;
	REQUIRE(tmp[1][1] == 45);
}
